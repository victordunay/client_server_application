...
typedef cuda::atomic<int> atomic_lock_t;

__device__ atomic_lock_t* debug_lock;
__device__ void init_lock()
{ debug_lock = new atomic_lock_t(0);};
...
private:
    ...
    {
        printf("request lock !\n");
        while(_lock->exchange(1));
        printf("request lock done !\n");
    }
...
public:
    ...
    __device__  __host__ bool dequeue_request(int *img_id)
    {
        bool qIsEmpty = true;
        *img_id = INIT_ID;
        ...
        if(!qIsEmpty)
        {
            ...
            /* consider uncomment for potimization
            {
                Unlock(_readerlock);
                Lock(_readerlock);
...
__global__ void consumer_proccessor(shared_queue *gpu_to_cpu_q,shared_queue *cpu_to_gpu_q,uchar *in, uchar *out, uchar* maps)
{
    int tid = threadIdx.x;
    int img_id = 0;
    bool res = false;
    ...
    while(1)
    {
        ...
        if(0 == tid)
        {
            res = cpu_to_gpu_q->dequeue_request(&img_id);
        }
        __syncthreads();
...
public:
    queue_server(int threads)
    {
        ...
        // TODO launch GPU persistent kernel with given number of threads, and calculated number of threadblocks
         ...
         consumer_proccessor<<<threadblocks, GRID_SIZE>>>(gpu_to_cpu_q,cpu_to_gpu_q,this->image_in,this->image_out, this->maps); 
    ...
    bool dequeue(int *img_id) override
    {
        ...
        return gpu_to_cpu_q->dequeue_request(img_id);
    }
};
